...
const char *faceCascadeFilename = "data/haarcascades/haarcascade_frontalface_alt.xml";
int iteration=0;
...
int maxnote[4]={0,0,0,0};   // array used for total note matching4
int Notes[4]={50,100,500,1000};
...
int loadTrainingData(CvMat ** pTrainPersonNumMat)
{
	...
	fileStorage = cvOpenFileStorage( "./backrupeedatabase/facedata.xml", 0, CV_STORAGE_READ );
	if( !fileStorage ) {
...
void storeTrainingData()
{
	...
	// create a file-storage interface
	fileStorage = cvOpenFileStorage( "./backrupeedatabase/facedata.xml", 0, CV_STORAGE_WRITE );
...
CvMat* retrainOnline(void)
{
	...
	learn("./backrupeedatabase/train.txt");
...
void recognizeFromCam(void)
{
	...
	printf("hello Recognizing person in the camera ...\n");
	...
	while (1)
	{
		...
		switch (keyPressed) {
			...
			case 't':	// Start training
				...
				trainFile = fopen("./backrupeedatabase/train.txt", "a");
		...
		//cvNamedWindow("camImg",1);
		//cvShowImage("camImg",camImg);
		printf("1called...");
		int angle=0;
			for(int ro=0;ro<1;ro++)
			{
				rotimg=cvCreateImage(cvGetSize(camImg),8,3);
			cvCopyImage(camImg,rotimg,0);
			rotimg=rotateImage(rotimg,angle);
			greyImg = convertImageToGreyscale(rotimg);
			faceRect = detectBackRupee(rotimg );
			//angle+=5;
			printf("3called...");
			//printf("%d %d ",faceRect.height,faceRect.width);
		...
		for(int p=0;p<4;p++)
		{
			if(maxnote[index_1]<maxnote[p])
			{
				index_1=p;
			}

		}
		if(maxnote[index_1]!=0)
